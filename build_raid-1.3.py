...
def stop_raid():
    ...
    # check md0 has existed in /proc/mdstat ?
    status = shell_cmd('cat /proc/mdstat | grep -E "^md0"', True, 1)
    if status == 0:
        is_start_2 = True

    if is_start_1 or is_start_2:
        status = shell_cmd('mdadm -S /dev/md0', True, 1)
        if status == 0:
            tips('stop md0 success')
        else:
            tips('stop md0 failed')
    else:
